    ...
    public void runOpMode() throws InterruptedException {
        ...
        if (TuningOpModes.DRIVE_CLASS.equals(MecanumDrive.class)) {
            ...
            switch (snapshotAnalysis) {
                case LEFT: {
                    //Turning needs to be tuned. Create OutTake and Intake subsystems,
                    Actions.runBlocking(
                            drive.actionBuilder(new Pose2d(-36,64, 1.57))
                                    ...
                                    .build());
                    Actions.runBlocking(
                            drive.actionBuilder(new Pose2d(-36, 34.5, 1.57))
                                    .turn(3.15 / 2)
                                    .waitSeconds(1)
                                    .build());
                    ...
                    Actions.runBlocking(
                            drive.actionBuilder(new Pose2d(50,34.5,1))
                                    .strafeToConstantHeading(new Vector2d(50, 34.5))
                                    .waitSeconds(1)
                                    .build());

                  //  LiftTargetHieght = Lift1.getCurrentPosition();
                  //  LiftTargetHieght = Lift2.getCurrentPosition();

                    //LiftTargetHieght = Lift1.getCurrentPosition();
                    //LiftTargetHieght = Lift2.getCurrentPosition();

                   /* Lift1.setPower(0); //Stopped Lift
                    if(CustomLiftPID.Calculate(Lift1.getCurrentPosition(),LiftTargetHieght,0.02)>0) {
                        Lift1.setPower(CustomLiftPID.Calculate(Lift1.getCurrentPosition(), LiftTargetHieght, 0.02));
                    }
                    if(CustomLiftPID.Calculate(Lift2.getCurrentPosition(),LiftTargetHieght,0.02)>0) {
                        Lift2.setPower(CustomLiftPID.Calculate(Lift2.getCurrentPosition(), LiftTargetHieght, 0.02));
                    } */
                ...
                case CENTER: {
                    //This Should Work. However, Create OutTake and Intake Subsystems.
                    Actions.runBlocking(
                            drive.actionBuilder(new Pose2d(-36,64, 1.57))
                                    .setTangent(0)
                                    .strafeToConstantHeading(new Vector2d(-36, 34.5))
                    ...
                    Actions.runBlocking(
                      ...
                      /*  Lift1.setPower(0); //When we are all the way down we the intake automatically
                        LiftTargetHieght = Lift1.getCurrentPosition();
                        Lift2.setPower(0);
                        LiftTargetHieght = Lift2.getCurrentPosition();

                        Lift1.setPower(0.2); //Lifting
                        sleep(1000);
                        LiftTargetHieght = Lift1.getCurrentPosition();
                        Lift2.setPower(0.2); //Lifting
                        sleep(1000);
                        LiftTargetHieght = Lift2.getCurrentPosition();
